...
router.post('/notes/new-note', isAuthenticated, async (req, res) => { /*ruta para recibir los datos, si esta autenticado el usuario, cuando el 
  ...
  const { title, description, date} = req.body; //obtiene del req.body el title y description como constantes
  const errors = []; //se van a colocar los mensajes de errores en caso existan
  if(!title){ //validacion de titulo... si no hay un titulo
    errors.push({text: 'Inserte Nombre De Cliente'}); //muestra un mensaje si no hay un titulo
  }
  if(!description){ //validacion de la descripcion... si no hay una descripcion
    errors.push({text: 'Inserte Un Servicio'}); //muestra un mensaje si no hay una descripcion
  }
  if(!date){ //validacion de la descripcion... si no hay una descripcion
    errors.push({text: 'Inserte Fecha y hora'}); //muestra un mensaje si no hay una descripcion
  }
...
router.put('/notes/edit-note/:id', isAuthenticated, async (req,res) => { //
  const {title, description, date } = req.body; //guarda en una constante el titulo y la desripcion obtenidos
